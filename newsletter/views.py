...
class SubscribeView(View):
    def post(self, request):
        try:
            ...
            # Create new subscriber with optimized database operation
            ...
            # Send emails asynchronously using threading for better performance
            import threading
...
class CreateCampaignView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if action == 'save_and_send':
            # Send the campaign asynchronously for better performance
            import threading
            ...
            def send_campaign_async():
                try:
                    service = NewsletterService()
                    result = service.send_campaign(campaign.id)
                    print(f"Campaign sending result: {result}")
                except Exception as e:
                    print(f"Error sending campaign: {e}")
            
            # Start email sending in background thread
            email_thread = threading.Thread(target=send_campaign_async)
            email_thread.daemon = True
            email_thread.start()
            
            messages.success(request, f'Campaign "{campaign.name}" is being sent in the background! You can check the status in the dashboard. The process may take a few minutes depending on the number of subscribers.')
        
        return redirect('newsletter:admin_dashboard')

@method_decorator(staff_member_required, name='dispatch')
class DeleteCampaignView(LoginRequiredMixin, View):
    """View for deleting campaigns"""
    ...
    def post(self, request, campaign_id):
        try:
            ...
            # Optional: Prevent deletion of sent campaigns (uncomment if desired)
            # if campaign.status == 'sent':
            #     messages.warning(request, f'Cannot delete sent campaign "{campaign_name}". Only draft campaigns can be deleted.')
            #     return redirect('newsletter:admin_dashboard')
