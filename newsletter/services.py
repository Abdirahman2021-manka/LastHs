...
class NewsletterService:
    ...
    def send_campaign(self, campaign_id):
        ...
        try:
            ...
            for i in range(0, total_subscribers, batch_size):
                ...
                with ThreadPoolExecutor(max_workers=5) as executor:
                    ...
                    for subscriber in batch:
                        future = executor.submit(self._send_email_to_subscriber, campaign, subscriber)
                        futures.append((future, subscriber))
                    
                    # Collect results
                    for future, subscriber in futures:
                        try:
                            future.result()  # This will raise an exception if the email failed
                            sent_count += 1
            ...
            # Bulk update subscriber metrics for better performance
            if sent_count > 0:
                from django.db.models import F
                NewsletterSubscriber.objects.filter(
                    id__in=[sub.id for sub in subscriber_list[:sent_count]]
                ).update(total_emails_sent=F('total_emails_sent') + 1)
    ...
    def _get_target_subscribers(self, campaign):
        ...
        subscribers = NewsletterSubscriber.objects.filter(
            status='active',
            is_verified=True
        ).select_related().prefetch_related('subscription_interests')
